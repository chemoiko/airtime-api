...
from verify_email.email_handler import resend_verification_email
from .edoct_token_manager import TokenManager
from verify_email.forms import RequestNewVerificationEmail
...
from .models import DashboardUsers
...
def edoct_VerifyUser(useremail, usertoken):
    token_manager = TokenManager()

    verified_user = token_manager.decrypt_link(useremail, usertoken)
    if verified_user:
        dashboard_user = DashboardUsers.objects.get(user = verified_user)
        print("gotten db user: ", dashboard_user)
...
def verify_user_and_activate(request, useremail, usertoken):
    ...
    if request.method == 'GET':
        try:
            verified = edoct_VerifyUser(useremail, usertoken)
            #verify_user(useremail, usertoken)
            if verified is True:
                ...
                return render(
                    ...
                    context={
                        ...
                        'link': '/dashboard'#reverse(login_page)
                    }
                )
            else:
        ...
        if useremail is None or usertoken is None:
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    inactive_dashboard = DashboardUsers.objects.get(user=inactive_user)
                    if inactive_dashboard.is_active:
